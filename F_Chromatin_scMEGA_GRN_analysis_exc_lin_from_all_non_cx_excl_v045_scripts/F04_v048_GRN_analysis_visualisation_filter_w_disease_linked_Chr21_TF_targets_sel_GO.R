...
load(file = paste0(main_dir,"E_DESeq_pseudobulk_by_cluster_exc_lin_from_all_non_cx_excl_v045/",
                   "E03_bulk_data_w_expr_z_scores.rda"))

# get table of selected GO terms

go_tab = read_csv(paste0(main_dir,"E_DESeq_pseudobulk_by_cluster_exc_lin_from_all_non_cx_excl_v045/",
                    "E03_GO_results_vs_N_DEGs_by_cluster_selected.csv"))
...
t1 = data.frame(Group_TF_targets = c("Genes_Chr21_TF_targets", "Genes_not_Chr21_TF_targets"),
  Genes_ID_causal = c(length(genes_ID_Chr21_TF_targets), 
                      length(genes_ID)-length(genes_ID_Chr21_TF_targets)), 
  Genes_not_ID_causal = c(length(genes_Chr21_TF_targets) - length(genes_ID_Chr21_TF_targets), 
                                        length(genes_all) - length(genes_ID)- length(genes_ID_Chr21_TF_targets)))
...
pdf(file = paste0(out_dir,script_ind, "TF_targets_by_GO_term_heatmap_top_20_GO_terms_DS_reg.pdf"), width = 20, height = 10)
{
...
########################################################################
# plot TF targets vs sel GO term genes (only interactions consistent with DS-dependent regulation)
########################################################################
...
pdf(file = paste0(out_dir,script_ind, "TF_targets_by_GO_term_heatmap_sel_GO_terms_DS_reg.pdf"), width = 20, height = 10)
{
  pheatmap::pheatmap(tf_go_mat, show_rownames=TRUE, show_colnames = TRUE,
                     cluster_rows = FALSE, cluster_cols = FALSE,  
                     clustering_distance_rows = "euclidean",
