message("\n\n##########################################################################\n",
        "# Start F06: PPI interactions with scMEGA network core TFs ", Sys.time(),
        "\n##########################################################################\n",
        "\n   ",
        "\n##########################################################################\n\n")
...
for (i in 1:N_query_genes){
  ...
  resp <- GET(url2)
  l2 <- httr::content(resp, as = "text") %>% fromJSON()
...
#add vst mean of control samples

m1 = bulk_data$vst_mat
m2 = m1[match(PPI_nodes$gene, rownames(m1)), meta$cluster_sample[meta$group == "CON"]]
mean_vst_CON = apply(m2, 1, mean)

PPI_nodes$mean_vst_CON = mean_vst_CON

#add mean Z-scores CON and DELTA (mean DS - mean CON samples)
...
# plot interactions for all Chr21 genes (larger)
