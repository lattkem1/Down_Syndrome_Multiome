message("\n\n##########################################################################\n",
        "# Start J03a: DEG characterisation (reference mapped data)", Sys.time(),
        "\n##########################################################################\n",
        "\n   plot DEG stats per cluster, overlap ref vs mapped",
        "\n##########################################################################\n\n")
...
#load graft only DEseq2 dataset

load(file = paste0(out_dir,"J02a_bulk_data_with_DESeq_results.rda")) 
...
#################################################
# Plot number of DEG (incl Chr21 genes and number overlapping with tissue) by cluster
#################################################

bulk_data = bulk_data_mapped

DEGs_Chr21 = lapply(bulk_data$DEGs, intersect, GOI$Chr21)
meta = bulk_data$meta
meta$comp = meta$cluster_name
...
t1$N_genes = lengths(bulk_data$DEGs[paste0(t1$comp, "_DS_", t1$DS_up_down)])
t1$N_genes_Chr21 = lengths(DEGs_Chr21[paste0(t1$comp, "_DS_", t1$DS_up_down)])

for (i in 1:nrow(t1)){
  
  #genes up/down respectively in mapped cluster
  DEGs_cluster = bulk_data$DEGs[[paste0(t1$comp[i],"_DS_", t1$DS_up_down[i])]]
...
pdf(file = paste0(out_dir,script_ind,"DEGs_numbers_grafts_by_cluster.pdf"), 
    width = 4, height = 4)
{
  plot(p2)
  plot(p3)
...
DEG_overlap_list[["grafts_up"]] = unique(unlist(l2[grepl("_up", names(l2))]))
DEG_overlap_list[["foetal_up_grafts_up"]] = intersect(DEG_overlap_list[["foetal_up"]], 
                                                        DEG_overlap_list[["grafts_up"]])

DEG_overlap_list[["foetal_down"]] = unique(unlist(l1[grepl("_down", names(l1))]))
DEG_overlap_list[["grafts_down"]] = unique(unlist(l2[grepl("_down", names(l2))]))
DEG_overlap_list[["foetal_down_grafts_down"]] = intersect(DEG_overlap_list[["foetal_down"]], 
                                                            DEG_overlap_list[["grafts_down"]])

DEG_overlap_list[["foetal_up_grafts_down"]] = intersect(DEG_overlap_list[["foetal_up"]], 
                                                          DEG_overlap_list[["grafts_down"]])
DEG_overlap_list[["foetal_down_grafts_up"]] = intersect(DEG_overlap_list[["foetal_down"]], 
                                                          DEG_overlap_list[["grafts_up"]])
...
write_csv(as_tibble(m1), file = paste0(out_dir,script_ind, "DEGs_overlaps_foetal_grafts_genes.csv"))

t1 = tibble(gene_set = names(l3), N_genes = lengths(l3))

write_csv(t1, file = paste0(out_dir,script_ind, "DEGs_overlaps_foetal_grafts_N_genes.csv"))
...
pdf(file = paste0(out_dir,script_ind,"DEGs_overlaps_foetal_grafts.pdf"), 
    width = 4, height = 3.5)
{
  plot(p1)
}
...
pdf(file = paste0(out_dir,script_ind, "Gene_expr_heatmap_z_score_CON_DELTA_graft_tissue_overlap.pdf"), 
    width = 15, height = 15)
{
  
  p1 = bulkdata_heatmap(pl_mat = bulk_data$gene_Z_scores$mean_by_comp$DELTA, 
                        pl_meta = bulk_data$gene_Z_scores$mean_by_comp$meta,
                        pl_genes = DEG_overlap_list$foetal_up_grafts_up,
                        x_col = "comp", 
                        ...
                        fontsize = 10, 
                        title = paste0("Genes up tissue + grafts expression - Z-score mean DELTA"))
  
  pl_genes_clust = DEG_overlap_list$foetal_up_grafts_up[p1$tree_row$order]
  ...
  bulkdata_heatmap(pl_mat = bulk_data$gene_Z_scores$mean_by_comp$CON, 
                   ...
                   fontsize = 10, 
                   title = paste0("Genes up tissue + grafts expression - Z-score mean CON"))
  
  
  p1 = bulkdata_heatmap(pl_mat = bulk_data$gene_Z_scores$mean_by_comp$DELTA, 
                        pl_meta = bulk_data$gene_Z_scores$mean_by_comp$meta,
                        pl_genes = DEG_overlap_list$foetal_down_grafts_down,
                        x_col = "comp", 
                        meta_annot_cols = c("cluster_name", "sample_type"),
                        cluster_rows = TRUE, cluster_cols = FALSE,
                        color = colorRampPalette(c("magenta", "black", "yellow"))(250),
                        lims = lims_DELTA,  cellwidth = 10, cellheight = 10, 
                        fontsize = 10, 
                        title = paste0("Genes down tissue + grafts expression - Z-score mean DELTA"))
  
  pl_genes_clust =  DEG_overlap_list$foetal_down_grafts_down[p1$tree_row$order]
  ...
  bulkdata_heatmap(pl_mat = bulk_data$gene_Z_scores$mean_by_comp$CON, 
                   ...
                   lims = NULL,  cellwidth = 10, cellheight = 10, 
                   fontsize = 10, 
                   title = paste0("Genes down tissue + grafts expression - Z-score mean CON"))
