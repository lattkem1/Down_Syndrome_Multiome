...
out_dir = paste0(main_dir,"E_DESeq_pseudobulk_all_by_cluster/")
    ...
    width = 12, height = 30)
{
  for (go in names(go_genes_list)){
    ...
    m_DELTA = bulk_data$gene_Z_scores$mean_by_cluster_name$DELTA
    
    lims_DELTA = c(-0.1*max(abs(na.omit(m_DELTA))), 0.1*max(abs(na.omit(m_DELTA))))
    
    p1 = bulkdata_heatmap(pl_mat =  m_DELTA, 
                          pl_meta = bulk_data$gene_Z_scores$mean_by_cluster_name$meta,
                          pl_genes = go_genes,
                          x_col = "cluster_name", 
                          meta_annot_cols = c("cluster_name"),
...
#########################################
#plot genes by Hallmarks term (max top20 HPO terms)
#########################################

t1 = bulk_data$MSigDB_results
go_res = t1[t1$gs_cat == "H",]

go_genes_list = str_split(go_res$geneID, "/")
names(go_genes_list) = go_res$ID

pdf(file = paste0(out_dir,script_ind, "Gene_expr_heatmap_hallmarks_genes_z_score_mean_CON_DELTA.pdf"), 
    width = 12, height = 30)
{
  for (go in names(go_genes_list)){
