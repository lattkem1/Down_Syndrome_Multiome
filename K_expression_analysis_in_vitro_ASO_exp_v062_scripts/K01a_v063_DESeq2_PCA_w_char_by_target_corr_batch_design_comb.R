...
gr_tab$design_batch = paste0(gr_tab$ASO_design, "_", gr_tab$batch)

gr_tab$target_design = paste0(gr_tab$target,"_", gr_tab$ASO_design)

gr_tab = gr_tab[gr_tab$cell_line == "C13",]

gr_tab$group = gr_tab$target

gr_tab$group = str_remove_all(gr_tab$group, "none_")
...
comp_CON = "no_ASO"
comps = unique(gr_tab$group[gr_tab$group != comp_CON])
...
for (comp1 in comps){
  ...
  message("\n\n          *** DESeq2 analysis comp ", comp1, " - ", Sys.time(),"\n\n")
  ...
  bulk_data$deseq_dataset[[comp1]] = dds
...
t1 = bulk_data$deseq_results$BACH1
t2 = t1[t1$gene %in% c("PKNOX1","BACH1","GABPA"),]
...
t1$DS_up_down[grepl("up", t1$comp_group)] = "up"
t1$DS_up_down[grepl("down", t1$comp_group)] = "down"
...
### extract top genes driving PC

m1 = as_tibble(pc_analysis$x)

t1 = as_tibble(cbind(gene = rownames(pc_analysis$x), m1))

pc_list = list()

for (pc in colnames(pc_analysis$x)){
  
  t2 = t1[order(t1[[pc]]),]
  
  pc_list[[paste(pc,"_top100_neg")]] = t2$gene[1:100]
  
  t2 = t1[order(-t1[[pc]]),]
  
  pc_list[[paste(pc,"_top100_pos")]] = t2$gene[1:100]
  
}

t3 = data.frame(pc_list)

write_csv(t3, file = paste0(out_dir, script_ind, "PC_genes_top100_corr.csv"))

bulk_data$pca$pc_results = pc_analysis
bulk_data$pca$meta = meta_pca
bulk_data$pca$top_pc_genes = pc_list


#################################################
### GO-BP over-representation analysis of top genes PC1-4
#################################################

GO_list = list()
GO_results_tab = NULL

N_comps = length(bulk_data$pca$top_pc_genes)
names(bulk_data$pca$top_pc_genes)
N_comps = 8

for (i in 1:N_comps){
  
  comp = names(bulk_data$pca$top_pc_genes)[i]
  ...
  go_genes =  bulk_data$pca$top_pc_genes[[comp]]
...
write_csv(GO_results_tab, file = paste0(out_dir,script_ind, "GO_results_top100_PC_genes_corr.csv"))

bulk_data$pca$GO_results$by_comp_GO_list = GO_list
bulk_data$pca$GO_results$by_comp_GO_res = GO_results_tab
...
p1 = ggplot(t2, aes(x = comp, y = Description, size = Count, colour = comp))+
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
  labs(title = "Top10 GO terms per top PC genes")

pdf(file = paste0(out_dir,script_ind, "GO_results_by_PC_dotplot_top_terms_corr.pdf"), 
    width = 8, height = 10)
plot(p1)
dev.off()
...
pdf(file = paste0(out_dir,script_ind, "GO_results_by_PC_network_plot_corr.pdf"), 
    width = 15, height = 12)
lapply(pl, function(x){x})
dev.off()
...
#################################################
# plot heatmap DEGs combined by cluster_sample (smaller version)
#################################################
...
pdf(file = paste0(out_dir,script_ind, "Gene_expr_heatmap_DEGs_comb_small.pdf"), 
    width = 15, height = 20)
{
  p1 = bulkdata_heatmap(pl_mat = pl_mat_X, 
                        pl_meta = meta,
                        ...
                        fontsize = 10, title = paste0("Combined DEGs - Z-score"))
