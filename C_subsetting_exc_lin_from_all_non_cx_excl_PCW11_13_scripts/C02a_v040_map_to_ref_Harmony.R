...
out_dir = paste0(main_dir,"C_subsetting_exc_lin_from_all_non_cx_excl_PCW11_13/")
...
gr_tab = gr_tab[gr_tab$dev_PCW >=11 & gr_tab$dev_PCW <=13, ]
...
t3 = cbind(t2, area = "query", age = t1$dev_PCW+2, cell_type = "NA", 
...
pl[["cell_type_ref"]] = p1 + geom_point(aes(color = cell_type_ref), size = 0.5, alpha =0.7)+
...
pl[["group"]] = p1 + geom_point(aes(color = group), size = 0.5, alpha =0.7)+
...
pl[["area"]] = p1 + geom_point(aes(color = area), size = 0.5, alpha =0.7)+
  scale_color_manual(limits = c("query", predicted.areas), values = c("black", pal(predicted.areas)))+
...
pl[["area"]] = p1 + geom_point(aes(color = predicted.area), size = 0.5, alpha = 0.7)+
  scale_color_manual(limits = predicted.areas, values = pal(predicted.areas))+
  facet_wrap(facets = factor(pl_tab_shuffled$dataset, levels = datasets), nrow = 1)+
  labs(title = "precited area split by dataset")
...
area_pred_mat = matrix(nrow = length(samples), ncol = length(predicted.areas),
                       dimnames = list(samples, predicted.areas))

for (s1 in samples){
  
  t1 = stat_area_pred[stat_area_pred$sample == s1,]
  
  area_pred_mat[s1, t1$predicted.area] = t1$fract_sample_predicted_area
