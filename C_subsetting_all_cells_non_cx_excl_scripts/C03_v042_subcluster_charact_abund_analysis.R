...
load(file = paste0(out_dir, "C02_seur_integr.rda"))

#select subcluster resolution for further analyses
clust_res = 0.5

#get marker gene panels, TFs and Chr21 genes
GOI = list()
...
pl[["cluster_dist_box"]] = DimPlot(seur, group.by = "seurat_clusters", shuffle = TRUE, repel = TRUE, 
                               label.box = TRUE, label = TRUE, reduction = "umap", pt.size = 0.01)+
  scale_color_manual(limits = clusters, values = pal_dist(clusters))+
  NoLegend()+labs(title = "clusters")
...
pl[["cluster_name_umap_dist_no_label"]] = DimPlot(seur, group.by = "cluster_name", shuffle = TRUE, repel = TRUE, 
                                         label = FALSE, reduction = "umap", pt.size = 0.01)+
  scale_color_manual(limits = cluster_names, values = pal_dist(cluster_names))+
  labs(title = "cluster_names")

pl[["cluster_name_umap_dist_no_label_no_legend"]] = DimPlot(seur, group.by = "cluster_name", shuffle = TRUE, repel = TRUE, 
                                                  label = FALSE, reduction = "umap", pt.size = 0.01)+
  scale_color_manual(limits = cluster_names, values = pal_dist(cluster_names))+
  NoLegend()+labs(title = "cluster_names")

pl[["cluster_name_umap_dist_box"]] = DimPlot(seur, group.by = "cluster_name", shuffle = TRUE, repel = TRUE, 
                                              label.box = TRUE, label = TRUE, reduction = "umap", pt.size = 0.01)+
  scale_color_manual(limits = cluster_names, values = pal_dist(cluster_names))+
  scale_fill_manual(limits = cluster_names, values = pal_dist(cluster_names))+
  NoLegend()+labs(title = "cluster_names")
...
pdf(file = paste0(out_dir,script_ind, "Cell_markers_dotplot_clusters_labelled_cell_types.pdf"), 
    width = 10, height = 6)
plot(p1)
dev.off()

pdf(file = paste0(out_dir,script_ind, "Cell_markers_dotplot_clusters_labelled_subtypes.pdf"), 
    width = 8.5, height = 6)
plot(p2)
dev.off()
...
t1 = meta %>% group_by(cluster_name, sample) %>% dplyr::summarize(N_cells = n())
...
N_sample = stat_tab %>% group_by(sample) %>% dplyr::summarize(N = sum(N_cells))
...
N_cluster = stat_tab %>% group_by(cluster) %>% dplyr::summarize(N = sum(N_cells))
...
t2 = t1 %>% group_by(cluster, group) %>% 
  dplyr::summarise(mean_fract = mean(fract_sample), sd_fract = sd(fract_sample))
