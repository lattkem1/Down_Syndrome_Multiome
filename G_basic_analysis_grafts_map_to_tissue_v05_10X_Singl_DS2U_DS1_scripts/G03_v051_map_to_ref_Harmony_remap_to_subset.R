...
load(file = "C_subsetting_exc_lin_from_all_non_cx_excl/C03_seur_integr_labelled.rda") 
seur_ref = seur
rm(seur)
gc()
...
load(file = paste0(out_dir,"G02_seur_with_ref_mapping.rda")) 

# define predicted cell types to keep
t1 = read_csv("C_subsetting_all_cells_non_cx_excl/C02_subset_cluster_assignment_exc_lin.csv")

clust_keep = unique(t1$cluster_name)
...
seur = subset(x = seur,
              predicted.cluster_name %in% clust_keep)


t1 = seur@meta.data %>% group_by(library) %>% summarise(N_kept_for_analysis = n())
