...
#######################################################################################
### add plots split by group vs sample/library/batch_seq/dev_PCW/stage colored by cluster_name
#######################################################################################
...
t1 = FetchData(seur, vars = c("umap_1", "umap_2", "cluster_name","group", "sample","library", "stage", "dev_PCW", "batch_seq"))
...
#plot group vs library

p1 = ggplot(data = t1, aes(x = umap_1, y = umap_2, color = cluster_name))+geom_point(size = 0.05, alpha = 0.5)+
  scale_color_manual(limits = cluster_names, values = pal_dist(cluster_names))+
  ggplot2::facet_wrap(facets = factor(t1$library, levels = libraries), nrow = 2)+
  theme_bw()+
  labs(title = "UMAP by group vs library")+RestoreLegend()

pdf(file = paste0(out_dir, script_ind, "UMAP_split_by_group_vs_library.pdf"), width = 12, height = 2.5)
plot(p1)
dev.off()
