...
gr_tab_data = read_csv("A_input/group_tab_tissue.csv")
...
for (i in 1:nrow(gr_tab_data)){
  ...
  t1 = read_csv(paste0(gr_tab_data$dataset_folder[i],gr_tab_data$sample[i],"/outs/summary.csv"))
...
pdf(file = paste0(out_dir,script_ind,"cellranger_QC_barplots.pdf"), 
    width = nrow(gr_tab_data)/7+2, height = 40)
...
for (i in 1:nrow(gr_tab_data)){
  ...
  counts = 
    Read10X_h5(paste0(gr_tab_data$dataset_folder[i],gr_tab_data$sample[i],"/outs/filtered_feature_bc_matrix.h5"))
  ...
  fragpath = paste0(gr_tab_data$dataset_folder[i],
                    gr_tab_data$sample[i],"/outs/atac_fragments.tsv.gz")
