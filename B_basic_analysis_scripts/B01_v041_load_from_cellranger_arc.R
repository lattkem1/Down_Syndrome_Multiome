message("\n\n##########################################################################\n",
        "# Start B01: Load dataset into Seurat ", Sys.time(),
        "\n##########################################################################\n",
...
for (i in 1:nrow(gr_tab)){
  ...
  t1 = read_csv(paste0(gr_tab$dataset_folder[i],gr_tab$library[i],"/outs/summary.csv"))
...
pl =  lapply(plot_cols, function(plot_col){
  ...
  names(t2) = c("library", "pl_col")
  g1 = ggplot(t2, aes(x = library, y = pl_col))+geom_col()+
...
for (i in 1:nrow(gr_tab)){
  ...
  counts = 
    Read10X_h5(paste0(gr_tab$dataset_folder[i],gr_tab$library[i],"/outs/filtered_feature_bc_matrix.h5"))
  ...
  fragpath = paste0(gr_tab$dataset_folder[i],
                    gr_tab$library[i],"/outs/atac_fragments.tsv.gz")
...
pl[["nCount_RNA_max50k"]] = VlnPlot(seur, features = "nCount_RNA", group.by = "library", pt.size = 0.01) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  geom_hline(yintercept = c(500, 30000), colour = "red", linetype = 2, linewidth = 0.5)+
  NoLegend()+ ylim(0, 50000)+ labs(title = "nCount_RNA_max50k") 
pl[["nCount_RNA_max3000"]] = VlnPlot(seur, features = "nCount_RNA", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))  +
  geom_hline(yintercept = c(500, 30000), colour = "red", linetype = 2, linewidth = 0.5)+
...
pl[["percent.mt"]] = VlnPlot(seur, features = "percent.mt", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))  + 
  geom_hline(yintercept = c(2), colour = "red", linetype = 2, linewidth = 0.5)+
...
pl[["nCount_ATAC_max100k"]] = VlnPlot(seur, features = "nCount_ATAC", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  geom_hline(yintercept = c(100, 25000), colour = "red", linetype = 2, linewidth = 0.5)+
  NoLegend()+ ylim(0, 100000)+ labs(title = "nCount_ATAC_max100k")
pl[["nCount_ATAC_max3000"]] = VlnPlot(seur, features = "nCount_ATAC", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  geom_hline(yintercept = c(100, 25000), colour = "red", linetype = 2, linewidth = 0.5)+
  NoLegend()+ ylim(0, 3000)+labs(title = "nCount_ATAC_max3k")
pl[["TSS.enrichment_max10"]] = VlnPlot(seur, features = "TSS.enrichment", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  geom_hline(yintercept = c(1.1), colour = "red", linetype = 2, linewidth = 0.5)+
  NoLegend()+ ylim(0, 10)+labs(title = "TSS.enrichment_max10")
pl[["nucleosome_signal"]] = VlnPlot(seur, features = "nucleosome_signal", group.by = "library", pt.size = 0.01)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  geom_hline(yintercept = c(2), colour = "red", linetype = 2, linewidth = 0.5)+
  NoLegend()+labs(title = "nucleosome_signal")


pdf(file = paste0(out_dir,script_ind,"QC_plots.pdf"), width = 10, height = 3.5)
